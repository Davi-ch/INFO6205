...
public class MergeSort<X extends Comparable<X>> extends SortWithComparableHelper<X> {
    ...
    private void sort(X[] a, X[] aux, int from, int to) {
        ...
        // Recursively sort the left and right sides (note the swap of input and output arrays)
        sort(a, aux, from, mid);
        sort(a, aux, mid, to);

        // No need to merge if insurance optimisation is enabled and already ordered.
        if (insurance && helper.less(a[mid - 1], a[mid])) {
            return;
        }

        // perform merge (write a to aux)
        merge(a, aux, from, mid, to);

        // If not in noCopy mode, write aux back to the original array a
        if (!noCopy) {
    ...
    private void merge(X[] sorted, X[] result, int from, int mid, int to) {
        ...
        for (int k = from; k < to; k++) {
            if (i >= mid) {
                helper.copy(helper.get(sorted, j++), result, k);
            } else if (j >= to) {
                helper.copy(helper.get(sorted, i++), result, k);
            } else if (helper.less(sorted[j], sorted[i])) {
                helper.incrementFixes(mid - i); // <-- fixes/inversions
                helper.copy(helper.get(sorted, j++), result, k);
            } else {
                helper.copy(helper.get(sorted, i++), result, k);
            }
