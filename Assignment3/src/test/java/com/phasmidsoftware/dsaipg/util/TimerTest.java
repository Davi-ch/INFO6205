...
public class TimerTest {
    ...
    private void GoToSleep(long mSecs, int which) {
        try {
            ...
            long target = start + mSecs * 1_000_000;

            do {
                Thread.sleep(1);
            } while (System.nanoTime() < target);

            long actualSleepTime = (System.nanoTime() - start) / 1_000_000;

            if (which == 0) run++;
            else if (which > 0) post++;
            else pre++;

            System.out.println("Expected sleep: " + mSecs + " ms, Actual sleep: " + actualSleepTime + " ms");
